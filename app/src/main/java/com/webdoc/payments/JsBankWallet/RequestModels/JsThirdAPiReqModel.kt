package com.webdoc.payments.JsBankWallet.RequestModels


class JsThirdAPiReqModel {
    var processingCode: String? = null
    var merchantType: String? = null
    var traceNo: String? = null
    var companyName: String? = null
    var dateTime: String? = null
    var mobileNo: String? = null
    var channelId: String? = null
    var terminalId: String? = null
    var productId: String? = null
    var otpPin: String? = null
    var pinType: String? = null
    var transactionAmount: String? = null
    var reserved1: String? = null
    var reserved2: String? = null
    var reserved3: String? = null
    var reserved4: String? = null
    var reserved5: String? = null
    var reserved6: String? = null
    var reserved7: String? = null
    var reserved8: String? = null
    var reserved9: String? = null
    var reserved10: String? = null
}
